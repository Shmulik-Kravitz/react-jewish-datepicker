...
export const Day: React.FC<DayProps> = (props: DayProps) => {
    ...
    const handleMouseOver = () => {
        props?.onMouseOver && props?.onMouseOver(basicJewishDay);
    };

    const dayToDisplay = (isHebrew ? convertToHebrew(day, false, false) : day);
    ...
    const classNames = `day${otherMonthClass}${selectedDayClass || ""}${disableSelectClass}${isInRangClass}${isStartDayClass}${isEndDayClass}`;
    

    return (
        <div data-testid={isFromTest() ? title : undefined} data-date={props.date} className={classNames} title={title} onClick={handleClick} onMouseOver={handleMouseOver} >
            {dayToDisplay}
        </div>
    )
}
